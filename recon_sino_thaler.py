...
class ReconSino2:
    def __init__(self, sinogram_path, reconstructed_path, padding, _case):
    ...
    def reconstruct_astra(self, sinogram):
    ...
    def reconstruct_odl(self, sinogram):
        slices, header = nrrd.read(sinogram, index_order='C')
        rec = []
        for i in range(slices.shape[0]):
        # for i in range(16):
    ...
    def start_reconstruction(self):
        ...
        for dcm_file in tqdm(dcm_filenames):
            if dcm_file.endswith('.nrrd'):
                if self._case == 0:
                    recon_filename = os.path.join(self.reconstructed_path, 'noisy_astra_{:03d}.nrrd'.format(it))
                    nrrd.write(recon_filename,
                               self.reconstruct_astra(os.path.join(self.sinogram_path, dcm_file)),
                               index_order='C')
                if self._case == 1:
                    recon_filename = os.path.join(self.reconstructed_path, 'noisy_odl2_{:03d}.nrrd'.format(it))
                    nrrd.write(recon_filename,
                               self.reconstruct_odl(os.path.join(self.sinogram_path, dcm_file)),
                               index_order='C')
                it += 1

sinogram_path = "2dprojections/test_sino"
gt_path = "2dprojections/test_gt"
recon_path = "results_statistics/sparse_view_noisy/15_angles"
proba = ReconSino2(sinogram_path, recon_path, False, 1)
proba.start_reconstruction()